...
for (let s = 0; s < 4; s++)
  for (let v = 0; v < 8; v++)
    baseDeck.push({
      value: s > 1 ? RED_VALUE[v] : BLACK_VALUE[v],
      suit: SUIT[s],
    });
for (let v = 0; v < 22; v++)
  ...
  if (!document.getElementById("table").hidden) {
    ...
    for (let i = returnToDeck.length - 1; i >= 0; i--) {
      for (let j in returnToDeck[i].children) {
        if (
          returnToDeck[i].children[j] &&
          returnToDeck[i].children[j].nodeName == "IMG"
        ) {
...
function displayRoundInfo(theRoundInfo) {
  ...
  const possibleInfo = {
    contra: "Contra Multiplier: ",
    preverMultiplier: "Prever Multiplier: ",
  };
  const possiblePlayerNumbers = {
    povinnost: "Povinnost",
    prever: "Prever",
    valat: "Called Valat",
    iote: "Called I on the End",
  };
  let playerDivs = [];
  for (let i = 0; i < 4; i++) {
    ...
    if (theRoundInfo.pn - 1 == i) {
      theInfo.innerHTML += " (You)";
    }
    ...
    theAvatar.src =
      "/assets/profile-pictures/profile-" +
      (activeAvatars[i] ? activeAvatars[i] : 0) +
      ".png";
  ...
  for (let i in possibleInfo) {
    if (
      theRoundInfo[i] &&
      (i != "contra" || theRoundInfo[i] != 1) &&
      (i != "preverMultiplier" || theRoundInfo[i] != 1)
    ) {
      let theInfo = document.createElement("p");
      theInfo.classList.add("no-margin-below");
      theInfo.innerHTML =
        possibleInfo[i] +
        (isNaN(+theRoundInfo[i]) ? theRoundInfo[i] : +theRoundInfo[i]);
...
$(document).ready(function () {
  ...
  $(".copy-text").click(function (e) {
    ...
    if (navigator.share) {
      navigator
        .share({
          title: "Mach Tarok Invite",
          text: "Let's Play Taroky!",
          url: "machtarok.com/?join=CODE",
        })
...
function discardClickListener() {
  ...
  if (this.classList.contains("selected")) {
    ...
    numCardsSelected--;
    document.getElementById("discard_info").innerHTML =
      "Select " + (hand.length - numCardsSelected - 12) + " more cards";
  } else if (!this.classList.contains("grayed")) {
...
function displayNextAction(action) {
  ...
  if (action.player == playerNumber) {
    ...
    let playerName = activeUsernames[action.player]
      ? activeUsernames[action.player]
...
function startActionTimer() {
  if (
    ...
    isNaN(currentAction.time) ||
    !currentAction.time ||
    !theSettings ||
    isNaN(theSettings.timeout) ||
    theSettings.timeout <= 0
  ) {
    stopActionTimer();
    return;
  }
  ...
  switch (type) {
    ...
    case "timeout":
      socket.emit(
        "settings",
        type,
        document.getElementById("timeoutButton").value * 1000
      );
      break;
    case "lock":
    case "visibility":
      socket.emit(
        "settings",
        "lock",
        document.getElementById("lockButton").innerHTML == "Private"
      );
      break;
    case "aceHigh":
      socket.emit(
        "settings",
        type,
        document.getElementById("aceHighSelector").checked
      );
      break;
    case "botPlayTime":
      socket.emit(
        "settings",
        type,
        document.getElementById("botPlayTimeButton").value * 1000
      );
      break;
    case "botThinkTime":
      socket.emit(
        "settings",
        type,
        document.getElementById("botThinkTimeButton").value * 1000
      );
      break;
    case "save":
...
function hostRoom(roomSettings, roomName, joinCode) {
  ...
  document.getElementById("settingsRoomName").innerHTML =
    "Room " + romanize(roomName) + " (Join Code " + joinCode + ")";
...
function updateRoomSettings(roomSettings) {
  document.getElementById("display-difficulty").innerHTML =
    DIFFICULTY_TABLE[roomSettings.difficulty];
  document.getElementById("display-timeout").innerHTML =
    +roomSettings.timeout / 1000;
  document.getElementById("display-ace").innerHTML = roomSettings.aceHigh
    ? "Yes"
    : "No";
  document.getElementById("display-visibility").innerHTML = roomSettings.locked
    ? "Private"
    : "Public";
  document.getElementById("display-bot-play-time").innerHTML =
    +roomSettings.botPlayTime / 1000;
  document.getElementById("display-bot-think-time").innerHTML =
    +roomSettings.botThinkTime / 1000;

  if (document.getElementById(DIFFICULTY_TABLE[roomSettings.difficulty])) {
    document
      .getElementById(DIFFICULTY_TABLE[roomSettings.difficulty])
      .setAttribute("selected", "selected");
  }
  document
    .getElementById("timeoutButton")
    .setAttribute("value", roomSettings.timeout / 1000);
  ...
  document.getElementById("lockButton").innerHTML = roomSettings.locked
    ? "Private"
    : "Public";
  document
    .getElementById("botPlayTimeButton")
    .setAttribute("value", roomSettings.botPlayTime / 1000);
  document.getElementById("botPlayTimeButton").value =
    roomSettings.botPlayTime / 1000;
  document
    .getElementById("botThinkTimeButton")
    .setAttribute("value", roomSettings.botThinkTime / 1000);
  document.getElementById("botThinkTimeButton").value =
    roomSettings.botThinkTime / 1000;
}
...
function updatePlayersInGame(playersInGame) {
  ...
  for (let i in playersInGame) {
    document.getElementById("settingsScreenPn" + (+i + 1)).innerHTML =
      playersInGame[i].name + (i == pn ? " (You)" : "");
    activeAvatars[i] = playersInGame[i].avatar ? playersInGame[i].avatar : 0;
    if (document.getElementById("playerAvatar" + (+i + 1))) {
      document.getElementById("playerAvatar" + (+i + 1)).src =
        "/assets/profile-pictures/profile-" +
        (playersInGame[i].avatar ? playersInGame[i].avatar : 0) +
        ".png";
    }
  }
}
...
window.addEventListener(
  ...
  (event) => {
    if (
      event.origin !== "https://sso.smach.us" &&
      event.origin !== "https://sso.samts.us"
    ) {
...
function onLoad() {
  ...
  document.addEventListener(
    "mozfullscreenchange",
    renderer.fullscreenChangeEvent
  );
  document.addEventListener(
  ...
  }
  {
    ...
    if (theUsername && theToken) {
      socket = io({
        auth: {
          token: localStorage.getItem("tarokyInstance"),
          username: theUsername,
          signInToken: theToken,
        },
      });
    } else {
  ...
  socket.on("autoReconnect", function (data) {
    ...
    if (typeof data.playerCount !== "undefined") {
      ...
      document.getElementById("online-s").innerHTML =
        data.playerCount == 1 ? "" : "s";
    }
    ...
    if (typeof data.settings !== "undefined") {
      ...
      addBoldMessage(
        "Playing on difficulty " +
          DIFFICULTY_TABLE[data.settings.difficulty] +
          " with timeout " +
          data.settings.timeout / 1000 +
          " with ace high " +
          (data.settings.aceHigh ? "enabled" : "disabled")
      );
    }
    ...
    if (
      typeof data.nextAction !== "undefined" &&
      data.nextAction.action != "start"
    ) {
  ...
  socket.on("returnPlayerList", function (returnPlayerList) {
    ...
    inviteTable.innerHTML =
      '<thead><tr><th scope="col">Username</th><th scope="col">Status</th><th scope="col">Invite</th></tr></thead>';
    ...
    for (let i in returnPlayerList) {
      ...
      inviteButton.addEventListener(
        ...
        function () {
          ...
          document.getElementById(this.id.split("i")[0] + "status").innerHTML =
            "Invited";
    ...
    if (returnPlayerList.length == 0) {
      ...
      document
        .getElementById("inviteNoOneOnline")
  ...
  socket.on("returnSettings", function (returnSettings) {
    ...
    addBoldMessage(
      "Playing on difficulty " +
        DIFFICULTY_TABLE[returnSettings.difficulty] +
        " with timeout " +
        returnSettings.timeout / 1000 +
        " with ace high " +
        (returnSettings.aceHigh ? "enabled" : "disabled")
    );
    updateRoomSettings(theSettings);
  });
  socket.on("returnPlayersInGame", function (returnPlayersInGame) {
    console.log(
      "Return players in game received " + JSON.stringify(returnPlayersInGame)
    );
        ...
        switch (extraInfo.step) {
          ...
          case 2:
            if (theMessage == "") {
              ...
              if (
                extraInfo &&
                extraInfo.youMessage &&
                extraInfo.pn == playerNumber
              ) {
                addBoldMessage(extraInfo.youMessage);
                if (extraInfo.step == 2) {
      ...
      case MESSAGE_TYPE.NOTATION:
        addMessage(
          "Game save code: " +
            theMessage +
            ";pn=" +
            playerPerspective(playerNumber, extraInfo.povinnost)
        );
        break;
      case MESSAGE_TYPE.CUT:
        if (extraInfo && extraInfo.youMessage && extraInfo.pn == playerNumber) {
          addBoldMessage(extraInfo.youMessage);
        } else {
          addBoldMessage(theMessage);
  ...
  socket.on("autoAction", function (theAction) {
    addBoldMessage(
      "Your play timed out and was automatically completed for you"
    );
  ...
  socket.on("failedDiscard", function (toDiscard) {
    addError(
      "Failed to discard the " + toDiscard.value + " of " + toDiscard.suit
    );
  });
  ...
  socket.on("defaultSettings", function (returnSettings) {
    if (returnSettings) {
      console.log("Default settings received ", returnSettings);
...
function romanize(num) {
  ...
  if (num == 4) {
    return "IIII";
  } //Taroky cards have IIII instead of IV
...
function joinAudience(event) {
  ...
  if (!connectingToRoom) {
    ...
    socket.emit("joinAudience", this.roomID);
    addMessage("Joining audience in room " + this.roomID + "...");
  } else {
    addError("Already connecting to a room!");
  }
}
...
function buttonClick() {
  if (!connectingToRoom) {
    this.firstChild.innerHTML = "⟳";
    connectingToRoom = true;
    socket.emit("roomConnect", this.roomID);
    addMessage("Connecting to room " + this.roomID + "...");
  } else {
    addError("Already connecting to a room!");
  }
}
...
function customRoomClick() {
  ...
  if (!connectingToRoom) {
    ...
    generateNotationButton.addEventListener("click", () => {
      document.getElementById("notationInputField").value =
        generateRandomNotationSequence(
          document.getElementById("notationWeightSlider").value / 100
        );
    });
    ...
    notationInputFieldP.innerHTML =
      "Room Notation (Press the ⟳ symbol to edit the hand before beginning):";
    ...
    notationSubmitButton.addEventListener(
      "click",
      notationSubmitButtonClickEvent
    );
...
function loadCardsFromRoomCode() {
  if (!connectingToRoom) {
    ...
    let theNotationSettings =
      theNotationSplit[theNotationSplit.length - 1].split(";");
    let thePN =
...
function generateRandomNotationSequence(goodHandWeight) {
  ...
  for (let i in workingDeck) {
    workingDeck[i].weight =
      (VALUE_REVERSE[workingDeck[i].value] +
        (workingDeck[i].value == "I" ? 15 : 0)) *
      (workingDeck[i].suit == "Trump" ? 3 : 1);
  }
...
function notationSubmitButtonClickEvent() {
  if (!connectingToRoom) {
    ...
    if (document.getElementById("notationSubmitButton").generated) {
      ...
      for (let i = 0; i < 4; i++) {
        splitNotation[i + 4] = imageCollectionToNotation(
          document.getElementById("customHandInner" + i)
        );
      }
      splitNotation[8] = imageCollectionToNotation(
        document.getElementById("talonInner")
      );
      notation = "";
...
function createPartnerButtons(possiblePartners) {
  for (let i in possiblePartners) {
    ...
    button.classList.add("choice-button");
    button.innerHTML =
      possiblePartners.length > 1 && possiblePartners[i].value == "XIX"
        ? "Alone (XIX)"
        : "With Partner (" + possiblePartners[i].value + ")";
    button.id = possiblePartners[i].value;
    button.addEventListener("click", () => {
      partnerButtonsOnClickListenerTasks(
        possiblePartners[i].value,
        possiblePartners
      );
    });
    document.getElementById("center").appendChild(button);
...
function partnerButtonsOnClickListenerTasks(cardValue, possiblePartners) {
  ...
  for (let i in possiblePartners) {
    document
      .getElementById("center")
      .removeChild(document.getElementById(possiblePartners[i].value));
  }
}
...
function partnersReturned(possiblePartners) {
  ...
  if (partners.length > 1) {
    ...
    for (let i in partners) {
      partnerString += partners[i].value + ", ";
    }
    addMessage(
      "You can partner with your choice of the " +
        partnerString.substring(0, partnerString.length - 2)
    );
...
function confirmButtonCallback() {
  ...
  if (hand.length - numCardsSelected == 12) {
    ...
    addError(
      "Please select " +
        (hand.length - numCardsSelected - 12) +
        " more card" +
        (hand.length - numCardsSelected - 12 == 1 ? "" : "s")
    );
  }
}
...
function buttonChoiceCallback() {
  ...
  document
    .getElementById("center")
    .removeChild(document.getElementById("go" + TYPE_TABLE[buttonType]));
  document
    .getElementById("center")
    .removeChild(document.getElementById("no" + TYPE_TABLE[buttonType]));
}
...
function resetBoardButton() {
  ...
  theButton.addEventListener("click", () => {
    document
      .getElementById("center")
      .removeChild(document.getElementById("resetBoard"));
...
function createShareButton(score) {
  ...
  theButton.addEventListener("click", () => {
    ...
    const shareTitle =
      "I made a " +
      renderer.gamestate.dailyChallengeScore +
      " on MachTarok Daily!";
    const shareText =
      "I made a " +
      renderer.gamestate.dailyChallengeScore +
...
function exitCurrentRoom(value) {
  if (!value) {
    ...
    document
      .getElementById("exit")
      .setAttribute("onclick", "exitCurrentRoom(true)");
    exitTimeout = setTimeout(() => {
      document.getElementById("exit").innerHTML = "➤";
      document
        .getElementById("exit")
        .setAttribute("onclick", "exitCurrentRoom()");
    }, 10000);
  } else {
...
function displaySignIn() {
  renderer.hud.nav.renderSignIn(
    "https://sso.smach.us/?redirect=https://machtarok.com/"
  );
}
